...
try{
$name=trim((string)($_POST['customer_name']??''));$phone=trim((string)($_POST['phone']??''));$email=trim((string)($_POST['email']??''));$address=trim((string)($_POST['address']??''));$notes=trim((string)($_POST['notes']??''));$limit=(float)($_POST['credit_limit']??0);
...
$st=$conn->prepare("INSERT INTO customers (customer_name,phone,email,address,credit_limit,balance,status,notes) VALUES (?,?,?,?,?,0,'Active',?)");$st->execute([$name,$phone,$email?:null,$address?:null,$limit,$notes?:null]);
