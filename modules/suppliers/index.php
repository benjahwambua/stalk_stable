...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'create' || $action === 'update') {
            ...
            if ($action === 'create') {
                $stmt = $conn->prepare("INSERT INTO suppliers (supplier_name, contact_person, phone, alternative_phone, email, address, tax_pin, credit_limit, balance, status, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?)");
                $stmt->execute([$name,$contact,$phone,$altPhone,$email ?: null,$address,$taxPin,$creditLimit,$status,$notes]);
                $_SESSION['flash'] = ['type'=>'success','message'=>'Supplier created successfully.'];
            } else {
                $stmt = $conn->prepare("UPDATE suppliers SET supplier_name=?, contact_person=?, phone=?, alternative_phone=?, email=?, address=?, tax_pin=?, credit_limit=?, status=?, notes=? WHERE id=?");
