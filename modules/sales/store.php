...
$method=(string)($_POST['payment_method']??'Cash');$priceType=(string)($_POST['price_type']??'normal');if(!in_array($priceType,['normal','wholesale'],true))$priceType='normal';
...
$lines=[];$subtotal=0.0;
foreach($cart as $item){$pid=(int)($item['id']??0);$qty=(float)($item['qty']??0);if($pid<=0||$qty<=0||$qty!=round($qty))throw new RuntimeException('Invalid product quantity.');$s=$conn->prepare("SELECT * FROM products WHERE id=? AND status='Active' FOR UPDATE");$s->execute([$pid]);$p=$s->fetch();if(!$p)throw new RuntimeException('A selected product is unavailable.');if((float)$p['stock_quantity']<$qty)throw new RuntimeException('Insufficient stock for '.$p['product_name'].'. Available: '.$p['stock_quantity']);$price=$priceType==='wholesale'?(float)$p['wholesale_price']:(float)$p['selling_price'];if($priceType==='wholesale'&&$price<=0)throw new RuntimeException('Wholesale price is not set for '.$p['product_name'].'.');$line=$price*$qty;$subtotal+=$line;$lines[]=['p'=>$p,'qty'=>$qty,'price'=>$price,'subtotal'=>$line];}
