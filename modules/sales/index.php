...
function updateCatalogPrices(){document.querySelectorAll('.product').forEach(card=>{card.querySelector('.normal-price').style.display=wholesale?'none':'inline';card.querySelector('.wholesale-price').style.display=wholesale?'inline':'none';});cart.forEach(x=>{const p=products.find(y=>y.id===x.id);if(p)x.price=selectedPrice(p,x.qty);});render();}
