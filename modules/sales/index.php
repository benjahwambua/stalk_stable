...
<span class="badge"><?=number_format((float)$p['stock_quantity'],0).' '.e($p['unit']?:'units')?></span><div class="name"><?=e($p['product_name'])?></div><small><?=e($p['brand_name']?:'Generic')?> · <?=e($p['sku']?:'No SKU')?></small><div class="price"><span class="normal-price">KES <?=number_format((float)$p['selling_price'],2)?></span><span class="wholesale-price" style="display:none">KES <?=number_format((float)$p['wholesale_price'],2)?></span></div>
</div>
<?php endforeach; ?>
</div>
</section>
...
<div class="field"><label>Price List</label><select name="price_type" id="price_type"><option value="normal">Normal Price</option><option value="wholesale">Wholesale Price</option></select></div>
<div class="field"><label>Customer</label><select name="customer_id" id="customer_id"><option value="">Walk-in Customer</option><?php foreach($customers as $c): ?><option value="<?=$c['id']?>" data-limit="<?=$c['credit_limit']?>" data-balance="<?=$c['balance']?>"><?=e($c['customer_name'])?><?= $c['phone']?' — '.e($c['phone']):'' ?></option><?php endforeach; ?></select></div>
...
let cart=[];
function selectedPrice(p){const type=document.getElementById('price_type').value;const wholesale=Number(p.wholesalePrice||0);return type==='wholesale'&&wholesale>0?wholesale:Number(p.normalPrice||p.price||0);}
function addProduct(id){const p=products.find(x=>x.id===id);if(!p||p.stock<=0)return;const price=selectedPrice(p);const row=cart.find(x=>x.id===id);if(row){row.price=price;if(row.qty<p.stock)row.qty++;}else cart.push({...p,price,qty:1});render();}
...
function setQty(i,v){cart[i].qty=Math.max(1,Math.min(Number(v)||1,cart[i].stock));render()}function removeItem(i){cart.splice(i,1);render()}
...
function updateCatalogPrices(){const wholesale=document.getElementById('price_type').value==='wholesale';document.querySelectorAll('.product').forEach(card=>{card.querySelector('.normal-price').style.display=wholesale?'none':'inline';card.querySelector('.wholesale-price').style.display=wholesale?'inline':'none';});cart.forEach(x=>{const p=products.find(y=>y.id===x.id);if(p)x.price=selectedPrice(p);});render();}
...
document.getElementById('productSearch').addEventListener('input',filterProducts);document.getElementById('productSearch').addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();const q=e.target.value.toLowerCase();const p=products.find(x=>x.barcode&&String(x.barcode).toLowerCase()===q)||products.find(x=>x.sku&&String(x.sku).toLowerCase()===q);if(p){addProduct(p.id);clearSearch();}}});document.getElementById('price_type').addEventListener('change',updateCatalogPrices);document.getElementById('discount').addEventListener('input',calc);document.getElementById('tax').addEventListener('input',calc);document.getElementById('paid_amount').addEventListener('input',calc);document.getElementById('payment_method').addEventListener('change',()=>{const m=document.getElementById('payment_method').value;document.getElementById('mpesaPhoneField').style.display=m==='M-Pesa'?'block':'none';if(m==='Credit'||m==='M-Pesa')document.getElementById('paid_amount').value=0;calc()});
