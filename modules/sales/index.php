...
<span class="badge"><?=number_format((float)$p['stock_quantity'],0).' '.e($p['unit']?:'units')?></span><div class="name"><?=e($p['product_name'])?></div><small><?=e($p['brand_name']?:'Generic')?> · <?=e($p['sku']?:'No SKU')?></small><div class="price">KES <?=number_format((float)$p['selling_price'],2)?></div>
</div>
<?php endforeach; ?>
</div>
</section>
...
<div class="field"><label>Payment</label><select name="payment_method" id="payment_method"><option>Cash</option><option>M-Pesa</option><option>Bank</option><option>Credit</option><option>Mixed</option></select></div>
...
<script>
const products=<?=json_encode(array_map(fn($p)=>['id'=>(int)$p['id'],'name'=>$p['product_name'],'sku'=>$p['sku'],'barcode'=>$p['barcode'],'price'=>(float)$p['selling_price'],'stock'=>(float)$p['stock_quantity'],'unit'=>$p['unit']],$products),JSON_UNESCAPED_UNICODE)?>;
let cart=[];
function addProduct(id){const p=products.find(x=>x.id===id);if(!p||p.stock<=0)return;const row=cart.find(x=>x.id===id);if(row){if(row.qty<p.stock)row.qty++;}else cart.push({...p,qty:1});render();}
